...
def test_chat_and_plan_non_json_returns_plain_reply():
    out = Reasoner(_FakeChat("plain text"), deployment="d").chat_and_plan(
        "hi", current_goals=[]
    )
    assert out["reply"] == "plain text"
    assert out["goals_updated"] is False
    assert "category_rules_proposed" not in out


def test_adjudicate_merchants_flags_same_and_different():
    reply = json.dumps(
        {
            "decisions": [
                {"merchant": "Potey Bard", "same": True},
                {"merchant": "Starbucks", "same": False},
            ]
        }
    )
    reasoner = Reasoner(_FakeChat(reply), deployment="gpt-4o-mini")
    out = reasoner.adjudicate_merchants(
        "Pottery Barn",
        [{"merchant": "Potey Bard"}, {"merchant": "Starbucks"}],
    )
...
def test_adjudicate_merchants_ignores_malformed_rows():
    reply = json.dumps(
        {
            "decisions": [
                {"merchant": "", "same": True},
                "not-a-dict",
                {"merchant": "Potey Bard"},  # missing "same" -> defaults False
            ]
        }
    )
    reasoner = Reasoner(_FakeChat(reply), deployment="gpt-4o-mini")
    out = reasoner.adjudicate_merchants("Pottery Barn", [{"merchant": "Potey Bard"}])
    assert out["decisions"] == [{"merchant": "Potey Bard", "same": False}]


def test_adjudicate_merchants_non_json_returns_empty():
    out = Reasoner(_FakeChat("oops"), deployment="d").adjudicate_merchants("X", [])
    assert out["decisions"] == []
